...
def format_date_components(date_str):
    '''Format date string to extract year, month, and day components'''
    if not isinstance(date_str, str) or not date_str.strip():
        return {'year': None, 'month': None, 'day': None, 'formatted': ''}
    
    # Extract year first
    year = extract_year(date_str)
    
    # Try to extract month and day using common patterns
    import re
    ...
    if match1:
        return {
            ...
            'formatted': f"{match1.group(1)}-{match1.group(2).zfill(2)}-{match1.group(3).zfill(2)}"
        }
    ...
    if match2:
        return {
            ...
            'formatted': f"{match2.group(3)}-{match2.group(1).zfill(2)}-{match2.group(2).zfill(2)}"
        }
    
    # If only year found, return just year
    if year:
        return {'year': year, 'month': None, 'day': None, 'formatted': str(year)}
...
def validate_year_against_master(year_value, master_row):
    """Validate collection year(s) against master collector date ranges"""
    if not year_value or year_value == '':
        return "No_Year"
    
    # Handle year ranges (e.g., "1985-1987")
    if isinstance(year_value, str) and '-' in year_value:
        try:
            start_year, end_year = map(int, year_value.split('-'))
            years_to_check = [start_year, end_year]
        except ValueError:
            return "Invalid_Year"
    else:
        # Single year
        try:
            years_to_check = [int(year_value)]
        except (ValueError, TypeError):
            return "Invalid_Year"
    
    # Check against date_range_FM column in master sheet
    date_range_fm = master_row.get('date_range_FM')
    if not date_range_fm or pd.isna(date_range_fm):
        return "No_Master_DateRange"
    
    # Check if any of the years fall within the master date range
    for year in years_to_check:
        if check_date_in_range(year, str(date_range_fm)):
            return "Valid|DateRange_Match"
    
    return "Invalid|Outside_DateRange"

def is_strict_match(match_data, country, year_value):
    """Determine if a match meets strict criteria for IRN assignment"""
    # Must have name match
    if not match_data['name_match'].startswith('True'):
        return False
    
    # Count validation criteria that pass
    validation_score = 0
    
    # ID match adds significant weight
    if match_data['id_match'].startswith('True'):
        validation_score += 3
    
    # Country match adds weight
    if (match_data['FM_Country_match'].startswith('True') or 
        match_data['HUH_Country_match'].startswith('True')):
        validation_score += 2
    
    # Date match adds weight
    if (match_data['FM_Date_match'].startswith('True') or 
        match_data['HUH_Date_match'].startswith('True')):
        validation_score += 2
    
    # Team match adds weight
    if match_data['collector_team_match'].startswith('True'):
        validation_score += 1
    
    # Require minimum validation score for IRN assignment
    # Name + ID = 3 points (sufficient)
    # Name + Country + Date = 4 points (sufficient)
    # Name + any 2 other criteria = sufficient
...
def process_secondary_collectors(example_df, master_df):
    ...
    for _, row in example_df.iterrows():
        ...
        for i in existing_collectors:
            ...
            # Format dates and extract components before IRN search
            min_date_formatted = format_date_components(str(min_date)) if pd.notna(min_date) else {'year': None}
            max_date_formatted = format_date_components(str(max_date)) if pd.notna(max_date) else {'year': None}
            
            # Get years for validation
            years = [d['year'] for d in [min_date_formatted, max_date_formatted] if d['year'] is not None]
...
def create_modified_max_sheet(example_file, primary_results_file, secondary_results_file, master_file, output_file):
    ...
    # Add Year column from date fields
    modified_df['Year'] = ''
    modified_df['Year_Validation'] = ''
    ...
    # Fill in primary collector data and year
    for idx, row in modified_df.iterrows():
        ...
        if len(years) == 2 and years[0] != years[1]:
            year_value = f"{min(years)}-{max(years)}"
        elif len(years) >= 1:
            year_value = years[0]
        else:
            year_value = ''
        modified_df.at[idx, 'Year'] = year_value
        
        # Validate year against primary collector's master data
        primary_match = top_primary[top_primary['barcode'] == barcode]
        if not primary_match.empty and year_value:
            # Find the master row for this collector
            master_irn = primary_match.iloc[0]['master_irn']
            if master_irn:
                master_match = master_df[master_df['Collector_irn'] == master_irn]
                if not master_match.empty:
                    validation_result = validate_year_against_master(year_value, master_match.iloc[0])
                    modified_df.at[idx, 'Year_Validation'] = validation_result
                else:
                    modified_df.at[idx, 'Year_Validation'] = 'No_Master_Data'
            else:
                modified_df.at[idx, 'Year_Validation'] = 'No_IRN_Match'
        else:
            modified_df.at[idx, 'Year_Validation'] = 'No_Year_Or_Match'
        
        # Find primary collector match
        primary_match = top_primary[top_primary['barcode'] == barcode]
        if not primary_match.empty:
            match = primary_match.iloc[0]
            # Only assign IRN if it's not empty (strict match passed)
            irn_value = match['master_irn'] if match['master_irn'] != '' else ''
            modified_df.at[idx, 'primary_master_irn'] = format_numeric_id(irn_value) if irn_value else ''
        ...
        for _, sec_match in secondary_matches.iterrows():
            ...
            if 2 <= collector_num <= 5:
                # Only assign IRN if it's not empty (strict match passed)
                irn_value = sec_match['master_irn'] if sec_match['master_irn'] != '' else ''
                modified_df.at[idx, f'secondary_{collector_num}_master_irn'] = format_numeric_id(irn_value) if irn_value else ''
    ...
    for _, row in example_df.iterrows():
        ...
        if not primary_match.empty:
            ...
            summary_data.append({
                'Barcode': barcode,
                'Collector_Number': 1,
                'Master_IRN': format_numeric_id(irn_value) if irn_value else '',
...
def _default_output_dir(input_path: str) -> str:
    p = Path(input_path)
    return str(p.parent / f"{p.stem}_validation_outputs")


def _print_instructions():
    banner = textwrap.dedent(
        """
        Collector Validation Tool
        -------------------------
        Validates primary and co-collectors against a Master Collector CSV, produces:
          - Primary validation results
          - Secondary validation results
          - Modified Max sheet with top results
          - Collector results summary

        Usage (PowerShell):
          python ForKimberly\\collectorValidation.py -i "<path>\\Example_MAX_Sheet.csv" -m "<path>\\Master_Collector(newmain).csv" -d ".\\run_outputs"
          python ForKimberly\\collectorValidation.py -i .\\v2\\Max_Validation\\FilesToValidate\\Example_MAX_Sheet.csv -m .\\v2\\Max_Validation\\Dates\\Validator\\Master_Collector(newmain).csv -d .\\out

        Notes:
          - Input can be CSV or XLSX. Master is typically CSV.
          - If -d/--outdir is omitted, a folder '<input>_validation_outputs' will be created next to the input.
          - Files created inside the folder:
              validation_results(Primary).csv
              validation_results(Secondary).csv
              Modified_Max_Sheet_with_Top_Results.csv
              Collector_Results_Summary.csv
        """
    ).strip()
    print(banner)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="collectorValidation.py",
        description="Validate collectors against a master collector file and generate reports.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent(
            """
            Examples (PowerShell):
              python collectorValidation.py -i "C:\\path\\Example_MAX_Sheet.csv" -m "C:\\path\\Master_Collector(newmain).csv" -d .\\run_outputs
              python collectorValidation.py -i .\\v2\\Max_Validation\\FilesToValidate\\Example_MAX_Sheet.csv -m .\\v2\\Max_Validation\\Dates\\Validator\\Master_Collector(newmain).csv -d .\\out
            """
        ).strip(),
    )
    parser.add_argument("-i", "--input", dest="input_file", required=False, help="Path to the input Example/MAX sheet (CSV/XLSX).")
    parser.add_argument("-m", "--master", dest="master_file", required=False, help="Path to the Master_Collector CSV/XLSX file.")
    parser.add_argument("-d", "--outdir", dest="output_dir", required=False, help="Directory to write all output files. Will be created if it doesn't exist.")
    ...
    if not args.input_file or not args.master_file:
    ...
    out_dir = args.output_dir or _default_output_dir(input_file)
    os.makedirs(out_dir, exist_ok=True)
    primary_out = str(Path(out_dir) / 'validation_results(Primary).csv')
    secondary_out = str(Path(out_dir) / 'validation_results(Secondary).csv')
    modified_out = str(Path(out_dir) / 'Modified_Max_Sheet_with_Top_Results.csv')
    summary_out = str(Path(out_dir) / 'Collector_Results_Summary.csv')
    ...
    print(f"  Secondary: {secondary_out}")
    print(f"  Modified:  {modified_out}")
    print(f"  Summary:   {summary_out}")
    print("-" * 20)

    # Run validation (primary and secondary)
    process_data(input_file, master_file, primary_out, secondary_out)

    print("\nValidation completed successfully!")
    print(f"Primary results:   {primary_out}")
    print(f"Secondary results: {secondary_out}")

    # Create modified Max sheet with top results
    create_modified_max_sheet(input_file, primary_out, secondary_out, master_file, modified_out)
    print(f"Modified Max sheet: {modified_out}")

    # Create collector results summary
    create_collector_results_summary(input_file, primary_out, secondary_out, summary_out)
    print(f"Collector summary: {summary_out}")